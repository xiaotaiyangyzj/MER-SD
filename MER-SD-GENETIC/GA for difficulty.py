...
# 染色体(某个个体) Q矩阵 薄弱知识点向量 习题的总数目 知识点的总数目
    ...
    sum = 0; Len = 1
    for i in range(item_n):
        if (chromosome[i] == 1 and abs(Item_diff[i] - User_diff[user_id]) <= 0.1): # 设定选择的难度要在一定阈值内
           sum += abs(Item_diff[i] - User_diff[user_id])
           Len += 1
    return sum / Len
    # for i in range(item_n):
...
def select(user_id, population, WK, item2knowledge, item_n, knowledge_n, Y, Item_diff, User_diff, mp): # 选择
    ...
    new_dict = sorted(mydict.items(), key=lambda x: x[0], reverse=False)  # True按照值类型降序排列 False升序排列
...
def get_weak_concepts(path, epsilon = 0.5):
    ...
    item = pd.read_csv("ASSISTments2017/item.csv")
...
def get_recommend_item(chromosome):
    num = 0
    for j in chromosome:
        if (j == 1):
            num += 1
...
def main(T = 1, Y = 200, gamma = 0.15, p_cross = 0.6, p_mutation = 0.01): # 遗传算法主函数 调用一系列其他函数
    ...
    path = 'ASSISTments2017/ASSISTments2017.csv' # 认知状态
    path2 = 'ASSISTments2017/ASSISTments2017.txt'
    ...
    for user_id in range(user_n):
        ...
        q.append(fit)
        num.append(get_recommend_item(population[0]))

        print('所有学生的平均习题难度为:', np.average(q))
...
f.write('ASSISTment2017 running time in MER-SD-GENETIC for difficulty is'+' '+str(time_sum)+'s'+' '+"\n")
